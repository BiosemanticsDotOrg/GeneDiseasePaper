...
public class ChemicalsFromKEGGdrug {
	
	public int generalChemicalVocID = -3000;
	public String generalChemicalVocName = "CHEMICAL";
	public int specificChemicalVocID = -3006;
	public String specificChemicalVocName = "KEGGDRUG";
      ...
      if (databaseLinks){
        if (line.startsWith(" ")){
          ...
          if (dbName.equals("DrugBank")){
        	  String drugbankId = ids[1].trim();
        	  if (drugbankId.contains(" ")){
        		  String[] drugbankIds = drugbankId.split(" ");
        		  databaseIds.add(new DatabaseID("DRUG", drugbankIds[0]));
        		  databaseIds.add(new DatabaseID("DRUG", drugbankIds[1]));
        	  }else databaseIds.add(new DatabaseID("DRUG", ids[1].trim()));
            }
...
//        Set definition. If longer than 1024 characters, then substring and add a full stop.
          databaseIds.add(new DatabaseID("KEGD", keggID));
