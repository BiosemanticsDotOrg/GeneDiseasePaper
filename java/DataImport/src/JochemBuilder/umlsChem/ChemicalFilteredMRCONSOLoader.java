...
import org.erasmusmc.dataimport.UMLS.UMLSFiltersBeforeOntologyCreation;
...
public class ChemicalFilteredMRCONSOLoader {
	public static void loadFromChemicalFilteredMRCONSO(Ontology ontology, String filename, List<String> log_output, String abbreviationsFile) {
		...
		while (fileIterator.hasNext()) {
			...
			if (line.length() != 0) {
				...
				} else if (voc.equals("MSH")){
					String meshString = columns[meshCol].trim();
					DatabaseID databaseID = new DatabaseID("MESH", meshString);
					ontology.setDatabaseIDForConcept(concept.getID(), databaseID);
				}
...
//					Replace double quotation mark with single if there is only one mark in the string
					...
					while (i < checkedTerm.length()){
						currentchar = checkedTerm.charAt(i);
						if (currentchar =='"'){
					...
					if (numberOfQuotationMarks==1){
						checkedTerm = checkedTerm.replace('"', '\'');
